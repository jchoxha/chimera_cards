...
import { sphere, capsule, box, disc, render } from './sprite3d.mjs';
...
function ironhide() {
  ...
  m.push(...box([0, 2.04, 0.5], [0.36, 0.36, 0.16], METAL));      // crisp breastplate
  m.push(...box([0, 1.6, 0.36], [0.42, 0.15, 0.14], TRIM));       // belt plate
  m.push(sphere([-0.22, 2.5, 0.56], 0.1, TRIM)); m.push(sphere([0.22, 2.5, 0.56], 0.1, TRIM)); // chest studs
  // ── PAULDRONS (big) ──
  ...
  m.push(disc([-0.5, 1.92, 0.66], [0.12, 0, 1], 0.62, TRIM));     // round shield rim (behind)
  m.push(disc([-0.5, 1.92, 0.72], [0.12, 0, 1], 0.5, METAL));     // shield face
  m.push(disc([-0.5, 1.92, 0.78], [0.12, 0, 1], 0.16, TRIM));     // central boss
  ...
  // crisp blocky stone maul head at the haft top
  m.push(...box([0.55, 3.72, -0.86], [0.26, 0.5, 0.28], STONE));  // stone block
  m.push(...box([0.55, 4.06, -0.86], [0.3, 0.16, 0.32], METAL));  // top striking face
  m.push(...box([0.55, 3.38, -0.86], [0.3, 0.16, 0.32], METAL));  // bottom striking face
  m.push(...box([0.55, 3.72, -0.55], [0.16, 0.2, 0.05], TRIM));   // band
