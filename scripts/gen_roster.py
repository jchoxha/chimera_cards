...
def generate(only=None):
    """Generate roster portraits. `only` = optional list of ids to (re)generate.
    Guarded under __main__ so importing STYLE/OUT/CREATURES never triggers the batch."""
    t0 = time.time()
    todo = [c for c in CREATURES if (not only or c[0] in only)]
    for i, (cid, subject) in enumerate(todo, 1):
        path = f"{OUT}/{cid}.png"
...
if __name__ == "__main__":
    # `python gen_roster.py` = all; `python gen_roster.py tidecaller maw` = just those.
